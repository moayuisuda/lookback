...
6. All configuration must be persisted via `localApi` using local files — do not use `localStorage`.
...
12. When writing commands, you must first read the existing code in `app/src/commands/index.ts` and `app/src/commands-pending`. Do not couple or modify the main application code without explicit permission.
