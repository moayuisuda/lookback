...
16→        - 存储结构：图片统一保存在 `images/` 子目录下，元数据仍在 `meta/` 中，前端通过相对路径生成本地访问 URL。
    ...
    - **模型校验与下载**: 应用启动后校验 storage 根目录下 `model/` 完整性；缺失时通过 IPC 发送进度事件给渲染进程，由渲染进程展示全局下载进度弹窗（`ModelDownloadModal`），主进程不再创建独立下载窗口。
