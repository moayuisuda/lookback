...
- **指令扩展**: 外部 command 加载时动态注入 i18n，并在 command 上下文提供 `useT` 供命令内调用；加载时清空模块缓存并禁用脚本缓存以反映最新内容。
- **Command 文案**: commands-pending 内部 UI 文案通过 command 自带 i18n 注入。

- **组件结构**:
  - `components/CommandPalette.tsx`: 全局 command 弹窗，支持指令与文本搜索，提供外部命令导入/删除入口与图片色调/颜色联合搜索与拼接导出入口。
  - `components/Gallery.tsx`: 左侧瀑布流图片展示与搜索。
    ...
    - 搜索输入：搜索条件变更后使用防抖重置分页并触发拉取，避免旧请求结果混入。
    - 默认展示：初始化与搜索统一为同一分页流程，先清空再追加。
    - 拉取实现：核心加载与分页状态由 `store/galleryStore.ts` 统一管理，组件只负责触发与渲染。
    - 拖拽与粘贴：
  ...
  - `components/Canvas.tsx`: 右侧无限画布，基于 SVG 实现，支持拖拽、缩放、移动与自动布局。
    ...
    - **临时文件管理**:
      - 临时图片存放在对应 Canvas 目录下的 `assets` 子目录。
      - 同名导入自动生成唯一文件名，避免覆盖已有资源。
    ...
    - 拖拽与粘贴：
      ...
      - 支持右键拖拽平移画布（与空格+左键相同）。
    - **文本节点**:
      - 通过命令面板内置命令在命令触发位置创建文本节点。
      - 双击文本节点进入编辑模式（Input Overlay，尺寸与缩放同步）。
      - 文本尺寸与选框实时同步，支持拖拽、缩放、删除、撤销。
      - 统一 State 管理（CanvasItem = Image | Text）。
    - 自动布局（Auto Layout）：
      - 采用 Masonry 布局算法（瀑布流），列数固定为 5，列间距按 20px 屏幕间距折算。
      - 排序逻辑：优先根据图片当前空间位置（Y 轴优先，X 轴次之）进行排序，再进行排布，以保留用户的视觉顺序。
      - 确保图片紧凑排列、互不重叠。
      - 重排仅改变位置，不改变图片当前缩放与旋转。
      - 即使图片尚未完全加载（无宽高信息），也具备默认占位处理。
    - 交互优化：
      - 操作模式：空格 + 左键拖拽或中键拖拽平移画布；右键拖拽平移画布；左键拖动进行框选（Box Selection），支持 Shift/Ctrl 多选。
      - 图片双击：双击图片触发视口 contain；空格抬起时，若有选中主项则对主项 contain（期间未发生鼠标按下），若无选中项则对整幅画布做 contain；右键单击恢复上一次视口。
      - 焦点反馈：激活时显示 Theme Brand Color 边框高亮。
      - 清空按钮：左上角提供 Clear 按钮，一键清空画布（支持撤销）。
      - 多选拖拽：多选后拖拽任意选中图片，会联动移动所有选中图片。
    - 拖拽过程仅响应当前拖拽项，避免多路拖拽事件干扰。
      - 框选命中：在世界坐标系下，基于 CanvasItem 的宽高与旋转（`getRenderBbox` 计算包围盒）与框选矩形做相交判断，缩放/平移后仍能稳定选中多张。
      - Minimap：缩略图使用图片主色渲染节点，便于快速识别。
    - Minimap 世界边界计算对文本节点使用统一缩放因子。
    - 选中态（基于 SVG）：
        - 选中框渲染：Image/Text 的选中矩形统一在 `CanvasNode` 内渲染，节点只提供选框几何信息。
        - 单图：使用 SVG `<rect>` 绘制选中高亮边框，右上角为删除按钮，左上角为水平翻转按钮，按钮通过 `CanvasControlButton` 以固定屏幕大小绘制，不随画布缩放变化。
        - 多选：绘制整体包围框，右上角展示删除按钮，左上角展示整体翻转按钮，右下角提供矩形缩放控制点，拖拽时按组计算缩放因子并写回各节点的坐标与尺寸。
        - 控制按钮全部为共享组件与样式，统一通过 SVG 事件驱动，完全移除对 `react-konva/Transformer` 的依赖。
        - 画布透明度仅作用于节点内容，选框与控制点保持清晰。
        - 控制按钮支持自定义鼠标指针样式，保证操作可感知。
      - **层级管理**: 单击选中时自动置顶。
...
- 画布支持从桌面直接拖入本地图片文件或文件夹（递归扫描图片），先自动导入到 Gallery，再添加到画布。
