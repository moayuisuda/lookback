...
- **启动流程**: `main.tsx` 会在 React 挂载前先 hydrate i18n 与 settings（如 pinMode / pinTransparent / sidebarWidth），减少启动闪动并确保窗口状态可恢复。

- **组件结构**:
  - `components/Gallery.tsx`: 左侧瀑布流图片展示与搜索。
    - 搜索逻辑：支持 Tag 严格匹配与 Tag + 文本混合搜索（先 Tag 过滤候选集，再在候选集内做向量检索）。
    - 颜色过滤：支持选择颜色后按图片主体色近似过滤，并可与 Tag/文本组合。
    - 搜索输入：使用本地状态 + 防抖 (Debounce 300ms) 处理。
    - 默认展示：无搜索词时直接渲染 `allImages`，避免出现空白列表。
    - 拖拽与粘贴：
    ...
    - Pin Mode：固定模式下，隐藏左侧 Gallery，仅保留 Canvas。窗口宽度会自动收缩以保持 Canvas 宽度不变，同时窗口会自动右移以保持右侧边缘位置不变（模拟“收起左侧”效果）。支持在全屏应用之上置顶显示。Pin 切换动画已优化，避免闪烁。
  - `components/Canvas.tsx`: 右侧无限画布，支持拖拽、缩放、移动与自动布局。
    - 数据持久化：画布内容实时保存到 LocalStorage/JSON 文件，图片作为引用存储（关联 Gallery 图片 ID）。
  ...
  - `components/TitleBar.tsx`: 自定义标题栏，包含窗口控制按钮与 Pin 模式。
...
- `actions`: 修改 `state` 的函数集合（包含 Pin 模式切换与画布自动布局逻辑）。
...
- 支持全局粘贴操作，根据当前激活区域（Gallery/Canvas）决定导入行为。
- 画布内容自动持久化，重启应用后恢复。
- 启用 Pin 模式时，窗口置顶显示，仅展示画布区域，背景保持深色（不透明），隐藏 Gallery。
